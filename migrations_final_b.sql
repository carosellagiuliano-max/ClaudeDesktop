-- ============================================
-- SCHNITTWERK Database Migration - PART B
-- All Views
-- Run this SECOND after Part A completes
-- ============================================
...
SELECT
  ...
  ROUND(
    ...
    NULLIF(COUNT(*) FILTER (WHERE status IN ('converted', 'cancelled')), 0) * 100,
    1
  ) AS conversion_rate
FROM waitlist
GROUP BY salon_id, preferred_date_from;
...
SELECT
  cf.salon_id,
  cf.staff_id,
  COALESCE(st.display_name, ps.first_name || ' ' || ps.last_name) as staff_name,
...
GROUP BY cf.salon_id, cf.staff_id, st.display_name, ps.first_name, ps.last_name;
...
LEFT JOIN LATERAL (
  ...
  ORDER BY aps.sort_order NULLS FIRST, aps.created_at
  LIMIT 1
) apsel ON true
LEFT JOIN services svc ON svc.id = apsel.service_id
