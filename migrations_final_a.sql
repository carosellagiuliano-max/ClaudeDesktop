...
CREATE POLICY "Staff can view their own blocks"
  ...
  USING (
    ...
    OR EXISTS (
      SELECT 1 FROM user_roles ur
      WHERE ur.profile_id = auth.uid()
      AND (ur.salon_id = staff_blocks.salon_id OR ur.salon_id IS NULL)
...
CREATE POLICY "Managers can manage skills"
  ...
  USING (
    EXISTS (
      SELECT 1
      FROM user_roles ur
      JOIN staff s2 ON (ur.salon_id = s2.salon_id OR ur.salon_id IS NULL)
      WHERE ur.profile_id = auth.uid()
      AND ur.role_name IN ('admin', 'manager', 'hq')
...
CREATE POLICY "Staff can view their own absences"
  ...
  USING (
    ...
    OR EXISTS (
      SELECT 1 FROM user_roles ur
      WHERE ur.profile_id = auth.uid()
      AND (ur.salon_id = staff_absences.salon_id OR ur.salon_id IS NULL)
      AND ur.role_name IN ('admin', 'manager', 'hq')
    )
  );
...
-- Partial unique index for one active policy per salon
CREATE UNIQUE INDEX idx_unique_active_policy_per_salon
  ON deposit_policies(salon_id)
  WHERE is_active = true;
...
BEGIN
  ...
  IF v_service_id IS NULL THEN
    RETURN QUERY SELECT false, 0, 'No service linked to appointment'::TEXT;
    RETURN;
  END IF;
...
USING (
  EXISTS (
    SELECT 1 FROM user_roles ur
    WHERE ur.profile_id = auth.uid()
      AND (ur.salon_id = appointment_deposits.salon_id OR ur.salon_id IS NULL)
  )
);
...
USING (
  EXISTS (
    SELECT 1 FROM user_roles ur
    WHERE ur.profile_id = auth.uid()
      AND (ur.salon_id = deposit_policies.salon_id OR ur.salon_id IS NULL)
      AND ur.role_name = 'admin'
  )
);
